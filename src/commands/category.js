...
module.exports = {
  ...
  aliases: ["category", "categoryUpdate"],
  args: true,
  cooldown: 5,
  async execute(message, args, client, collection) {
    ...
    if (gameName.toLowerCase() == "none") {
      collection
        .update({ ServerID: serverID }, { $set: { Category: "none" } })
        .then((res) => {
          const successMessage = new Discord.MessageEmbed()
            .setColor("#2ecc71")
            .setTitle(
              "You will not receive category updates, make sure to add favorite streamers"
            );
    ...
    if (gameID.length && gameName !== "none") {
      collection
        .update({ ServerID: serverID }, { $set: { Category: gameID } })
        .then(() => {
          const successMessage = new Discord.MessageEmbed()
            .setColor("#2ecc71")
