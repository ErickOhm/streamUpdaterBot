        ...
        try {
          client.channels.fetch(channelID).then((channel) => {
            channel.messages.fetch({ limit: 15 }).then((messages) => {
              ...
              let mostViewed = data[0];
              let index = usernameArray.indexOf(mostViewed.user_name);
              if (index == -1) {
                sendStreamer(
                  mostViewed,
                  client,
                ...
                if (data.length > 1) {
                  ...
                  let randStreamer = data[rand];
                  let index = usernameArray.indexOf(randStreamer.user_name);
                  if (index < 0) {
                    sendStreamer(
                      randStreamer,
                      client,
