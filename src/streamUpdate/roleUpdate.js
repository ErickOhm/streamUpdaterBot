function roleUpdate(prevState, newState, client, collection) {
    ...
    collection.find({ ServerID: newState.guild.id }).then((res) => {
        ...
        if (newActivity === ACTIVITY) {
            ...
            if (
                FILTER &&
                newActivityGame === FILTER &&
                prevActivityGame !== newActivityGame
            ) {
                let dServer = client.guilds.cache.find(
                    (guild) => guild.id === newState.guild.id
                );
                if (!dServer.roles.cache.find((r) => r.name === CHOSEN_ROLE))
                    return;
                let role = dServer.roles.cache.find(
                    (r) => r.name === CHOSEN_ROLE
                );
                let member = dServer.members.cache.find((m) => {
                    return m.user.id === newState.userID;
                });
                member.roles.add(role);
            } else if (
                !FILTER &&
                (!prevActivity || prevActivity !== newActivity)
            ) {
                // UPDATE ROLE
                let dServer = client.guilds.cache.find(
                    (guild) => guild.id === newState.guild.id
